...
std::map<int, SDL3Window*> SDL3Window::windows;
bool SDL3Window::exitRunLoop = false;

SDL3Window::SDL3Window(GameWindowHost *windowHost) : windowHost(windowHost)
{
    ...
    m_SDLWindow = SDL_CreateWindow("Surreal Engine", 640, 480, 0);
    if (!m_SDLWindow) {
    ...
    char const* const* extNames = SDL_Vulkan_GetInstanceExtensions(&extCount); // This shouldn't be deleted because SDL owns this array

    if (!extNames)
        SDLWindowError("Error while getting Vulkan extensions: " + std::string(SDL_GetError()));
    ...
    SDL_Vulkan_CreateSurface(m_SDLWindow, instance->Instance, nullptr, &surfaceHandle);
...
void SDL3Window::ExitLoop()
{
    exitRunLoop = true;
}

void SDL3Window::SDLWindowError(const std::string&& message) const
{
    throw std::runtime_error(message.c_str());
}

void SDL3Window::OnSDLEvent(SDL_Event& event)
{
    if (event.type >= SDL_EVENT_WINDOW_FIRST && event.type <= SDL_EVENT_WINDOW_LAST)
    {
...
void SDL3Window::SetWindowTitle(const std::string& text)
{
    SDL_SetWindowTitle(m_SDLWindow, text.c_str());
}

void SDL3Window::SetWindowFrame(const Rect& box)
{
    if (isWindowFullscreen)
    {
        // Fullscreen windows are handled with SDL_SetWindowFullscreenMode
        const SDL_DisplayMode* current_mode = SDL_GetWindowFullscreenMode(m_SDLWindow);
        SDL_DisplayMode desiredMode;

        desiredMode.w = box.width;
        desiredMode.h = box.height;
        desiredMode.refresh_rate = current_mode->refresh_rate;

        SDL_SetWindowFullscreenMode(m_SDLWindow, &desiredMode);
    }
    else
    {
...
void SDL3Window::SetClientFrame(const Rect& box)
{
    SetWindowFrame(box);
}
...
void SDL3Window::ShowFullscreen()
{
    SDL_SetWindowFullscreen(m_SDLWindow, SDL_TRUE);
    isWindowFullscreen = true;
}

void SDL3Window::ShowMaximized()
{
    SDL_MaximizeWindow(m_SDLWindow);
}

void SDL3Window::ShowMinimized()
{
    SDL_MinimizeWindow(m_SDLWindow);
}

void SDL3Window::ShowNormal()
{
...
void SDL3Window::Activate()
{
}

void SDL3Window::ShowCursor(bool enable)
{
    if (enable)
        SDL_ShowCursor();
    else
...
void SDL3Window::LockCursor()
{
    SDL_SetWindowGrab(m_SDLWindow, SDL_TRUE);
    SDL_SetRelativeMouseMode(SDL_TRUE);
}

void SDL3Window::UnlockCursor()
{
...
bool SDL3Window::GetKeyState(EInputKey key)
{
    if (key == IK_None)
        return false;
...
void SDL3Window::OnKeyboardInput(SDL_KeyboardEvent& event)
{
...
void SDL3Window::OnKeyboardTextInput(SDL_TextInputEvent& event)
{
    windowHost->OnWindowKeyChar(std::string(event.text));
}

void SDL3Window::OnMouseInput(SDL_MouseButtonEvent& event)
{
...
void SDL3Window::OnMouseWheel(SDL_MouseWheelEvent& event)
{
    EInputKey id = IK_None;
...
void SDL3Window::OnMouseMove(SDL_MouseMotionEvent& event)
{
...
Rect SDL3Window::GetWindowFrame() const
{
    int x, y, w, h;
...
Size SDL3Window::GetClientSize() const
{
    int width, height;
...
int SDL3Window::GetPixelWidth() const
{
    int width;
...
int SDL3Window::GetPixelHeight() const
{
    int height;
...
double SDL3Window::GetDpiScale() const
{
    // SDL3 has a new function for obtaining the DPI
    SDL_DisplayID displayID = SDL_GetDisplayForWindow(m_SDLWindow);

    if (displayID == 0)
        SDLWindowError("Error on SDL_GetDisplayForWindow(): " + std::string(SDL_GetError()));

    float scale = SDL_GetDisplayContentScale(displayID); // Directly returns the scale, e.g. 2.0 is 200% scale.

    if (scale <= 0.0f)
        SDLWindowError("Error while querying the display scale: " + std::string(SDL_GetError()));

    return scale;
}

std::vector<Size> SDL3Window::QueryAvailableResolutions() const
{
    ...
    // Then obtain the available resolutions
    int displayModesCount;
    const SDL_DisplayMode** displayModes = SDL_GetFullscreenDisplayModes(displayID, &displayModesCount);

    if (!displayModes)
        SDLWindowError("Error on SDL_GetFullscreenDisplayModes(): " + std::string(SDL_GetError()));

    for (int i = 0 ; i < displayModesCount ; i++)
    {
        // The data also includes refresh rate but we ignore it (for now?)
        Size resolution(displayModes[i]->w, displayModes[i]->h);
...
EInputKey SDL3Window::SDLScancodeToInputKey(SDL_Scancode keycode)
{
    switch (keycode) {
...
SDL_Scancode SDL3Window::InputKeyToSDLScancode(EInputKey inputkey)
{
    switch (inputkey) {
